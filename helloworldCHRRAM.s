...
chrreset: ;this code resets the CHR RAM (do not need to use it in a rom that uses CHR ROM)
ldy #$20
lda #$00
tax ;transfer the value in A to X
sta ppuaddr ;you need to write in ppuaddr 2 times, to form a 16-bit addr on VRAM. Ex: you want to go to the addr $21F3 of VRAM. First, write 21 to ppuaddr, and then write F3.
sta ppuaddr ;the CHR address goes from addr $00 to $1FFF on VRAM
:
sta ppudata ;store the value in A at the ppuaddr address location, using ppudata
inx
bne :-
dey
bne :-
...
copychr: ;this code copies the text font chr to CHR RAM (do not need to use it in a rom that uses CHR ROM)
lda #.lobyte(chrfont) ;loads the low byte of the address value of the chrfont in A
sta $00
lda #.hibyte(chrfont) ;loads the high byte of the address value of the chrfont in A
sta $01
lda #$00
sta ppuaddr
sta ppuaddr
...
chrfont: ;the CHR font data, in 2BPP format (2 bits per pixel; 4 colors per pixel)
         ;(this would be in the "CHARS" segment in a rom that uses CHR ROM)
